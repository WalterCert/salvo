...
public class SalvoController {
    ...
    private List<Long> getGamesList() {
    ...
    private ResponseEntity<Object> postPlayers(@RequestParam String username, @RequestParam String password) {
    ...
    private ResponseEntity<Object> newLoggedPlayerInGame(@PathVariable Long Id, Authentication authentication) {
        ...
        if (existingGame.getGamePlayers().size() == 2)
            ...
            existingGame.addGamePlayer(gpAux);
    ...
    private ResponseEntity<Object> postGame(Authentication authentication) {
        ...
        if (loggedPlayer == null)
            ...
            GamePlayer gamePlayer = new GamePlayer(new Date(), game, loggedPlayer);
            gamePlayerRepository.save(gamePlayer);
            return new ResponseEntity<>(Map.of("gpid", gamePlayer.getId()), HttpStatus.CREATED);
        }
    }
    ...
    private ResponseEntity<Map<String, Object>> getGameView(@PathVariable Long id, Authentication authentication) {
    ...
    private List<Map<String, Object>> makeGamesSalvoesList(Game game) {
        ...
        game.getGamePlayers().forEach(gamePlayer -> dto.addAll(makeSalvoList(gamePlayer.getSalvoes())));
        return dto;
    }
    ...
    private List<Map> getHits(GamePlayer self, GamePlayer opponent) {
        ...
        for (Salvo salvo : salvoesOrdenados) {
            ...
            List<String> salvoLocationsList = new ArrayList<>(salvo.getLocations());
    ...
    private String getGameState(GamePlayer self, GamePlayer opp) {
        ...
        if (self.getSalvoes().size() == opp.getSalvoes().size()) {
            ...
            if (totallyShipsSunk(self.getShips(), opp.getSalvoes())) {
                Score score = new Score(game, selfPlayer, 0.0f, new Date());
                if (!existScore(score, game)) {
                    scoreRepository.save(score);
                }return "LOST";
            }
            if (totallyShipsSunk(opp.getShips(), self.getSalvoes())) {
    ...
    private boolean totallyShipsSunk(Set<Ship> selfShips, Set<Salvo> oppSalvoes) {
        ...
        for (Salvo salvo : oppSalvoesOrdenados) {
            ...
            List<String> HitsCells = salvo.getLocations().stream()//Instancio una Lista. Obtengo los salvoes enemigos
